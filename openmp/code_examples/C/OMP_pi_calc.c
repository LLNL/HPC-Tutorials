 ...
 * FILE: OMP_pi_calc.c
 * DESCRIPTION:  
 *   Mone Carlo pi Calculation - C Version
 *   This program calculates pi using a "dartboard" algorithm.  See
 *   Fox et al.(1988) Solving Problems on Concurrent Processors, vol.1
 *   page 207.  
 * AUTHOR: unknown
 * REVISED: 02/23/12 Blaise Barney
 * Revised: 9/26/21 Don Frederick - for OpenMP parallelism
***************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <omp.h>
...
printf("Starting omp version of pi calculation using dartboard algorithm...\n");
srandom (5);            /* seed the random number generator */
avepi = 0;
#pragma omp parallel
#pragma omp for private (tid, pi, i)
for (i = 0; i < ROUNDS; i++) {
   /* Perform pi calculation on serial processor */
   tid = omp_get_thread_num(); 
   srandom(tid);  /* seed the random number generator */
   pi = dboard(DARTS);
